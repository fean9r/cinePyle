...
def extract_director(directors):
    if len(directors) > 1:
        raise Exception("FATAL ERROR. Too many directors.")
...
def scrape_single_film(href):
    ...
    # Ex: http://www.cinematheque.fr/seance/25041.html
...
def scrape_cinematheque_films(url_to_scrape):
    ...
    print ('## Scraping', len(show_hrefs), 'shows.')
    
    with concurrent.futures.ThreadPoolExecutor(max_workers=5) as executor:
        results = executor.map(scrape_single_film, show_hrefs)
...
class GeneralCalendar(object):
    ...
    def make_show(self, event):
        try :
            ...
            print('ERROR. Problem creating a Show', event)
    ...
    def getEvents(self, month):
        ...
        shows_m_i = list(map(lambda x: self.make_show(x), seances))
        return shows_m_i

def retreive_seances(start, end):
    all_seances = []
    months = [str(dt.month)+"-"+str(dt.year) for dt in rrule(MONTHLY, dtstart=start, until=end)]
    calendar = GeneralCalendar()
    for m_i in months:
        all_seances.append(calendar.getEvents(m_i))
    return all_seances
